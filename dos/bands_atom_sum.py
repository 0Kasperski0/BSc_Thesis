...
for filename in os.listdir(directory):
	if filename.endswith(".dat"):
		...
		if f_dop != "Vac":
			if f_conc=="0.0":
				ax.plot(x,total_sum,'black',linewidth=linewidth,label='SnTe') # numerki z pliku i nazwa domieszki też	
			else:
				ax.plot(x,total_sum,'black',linewidth=linewidth,label=(u'$\mathrm{Sn_{'+str(1-float(f_conc))+r'}}'+'\mathrm{'+f_dop+r'_{'+f_conc+r'}}\mathrm{Te}$')) # numerki z pliku i nazwa domieszki też
		else:
			ax.plot(x,total_sum,'black',linewidth=linewidth,label=(u'$\mathrm{SnTe}$'+ r' + '+str(100*float(f_conc))+r"% $\mathrm{Vac_{Sn}}$"))# numerki z pliku i nazwa domieszki też
		...
		ax.text(0, 14, r'$E_F$', fontsize=10,rotation=0)
